...
def detectar_fila_encabezado(xls, sheet_name, columnas_clave, max_filas=15):
    """Prueba header=0..max_filas-1 hasta encontrar una fila que contenga
    TODAS las columnas_clave (insensible a mayúsculas/espacios). Devuelve el
    índice de fila (0-based, como lo espera pandas `header=`) o None."""
    for header_row in range(max_filas):
        try:
            df_test = pd.read_excel(xls, sheet_name=sheet_name, header=header_row, nrows=5)
        except Exception:
            continue
        cols_upper = {str(c).strip().upper() for c in df_test.columns}
        if all(k in cols_upper for k in columnas_clave):
            return header_row
    return None
...
def leer_tim_wipcr(file):
    ...
    Detecta automáticamente en qué fila están los encabezados de cada hoja
    (no asume que siempre es la fila 1) — por si se movieron."""
    try:
    ...
    # ── WIP: buscar la fila donde están DISPO + ESTILO_EQ + LBS_C ──
    fila_wip = detectar_fila_encabezado(xls, 'WIP', {'DISPO', 'ESTILO_EQ', 'LBS_C'})
    if fila_wip is None:
        return None, None, "No encontré los encabezados de WIP (busqué DISPO/ESTILO_EQ/LBS_C en las primeras 15 filas).", {}
    df_wip = pd.read_excel(xls, sheet_name='WIP', header=fila_wip)
    df_wip.columns = df_wip.columns.str.strip().str.upper()

    # ── CUOTA: A/B no tienen título propio, así que buscamos la fila que
    # trae los títulos ABASTO/CUOTA (columnas I/K) para ubicar el encabezado ──
    fila_cuota = detectar_fila_encabezado(xls, 'CUOTA', {'ABASTO', 'CUOTA'})
    if fila_cuota is None:
        return df_wip, None, "No encontré los encabezados de CUOTA (busqué ABASTO/CUOTA en las primeras 15 filas).", {'fila_wip': fila_wip}
    df_cuota = pd.read_excel(xls, sheet_name='CUOTA', header=fila_cuota)
    cols = list(df_cuota.columns)
    if len(cols) < 2:
        return df_wip, None, "La hoja CUOTA no tiene columnas suficientes.", {'fila_wip': fila_wip, 'fila_cuota': fila_cuota}
    ...
    if 'ABASTO' not in df_cuota.columns or 'CUOTA' not in df_cuota.columns:
        return df_wip, None, "La hoja CUOTA no tiene columnas ABASTO/CUOTA reconocibles (revisa encabezados).", {'fila_wip': fila_wip, 'fila_cuota': fila_cuota}
    ...
    return df_wip, df_cuota, None, {'fila_wip': fila_wip, 'fila_cuota': fila_cuota}
...
if uploaded:
    df_wip, df_cuota, err, info_filas = leer_tim_wipcr(uploaded)
    if err:
        st.error(err)
        df_wip = None
    else:
        ...
        if faltan:
            ...
            c2.caption(f"WIP: {len(df_wip):,} filas · {df_wip['DISPO'].nunique():,} dispos "
                       f"(encabezado fila {info_filas['fila_wip'] + 1})")
            c3.caption(f"CUOTA: {len(df_cuota):,} combinaciones PLANTA_WIP+CONSTRUCCION "
                       f"(encabezado fila {info_filas['fila_cuota'] + 1})")
